...
def connect_as_user(
    ...
    *username* and *password* substituted into the URI.  Otherwise the URI is
    ...
    target_db = database or os.getenv("MONGO_INITDB_DATABASE") or os.getenv("DATABASE_NAME") or _DEFAULT_DB

    mongo_uri = os.getenv("MONGO_URI")
    if mongo_uri:
        # Substitute credentials into the template URI.
        uri = mongo_uri.replace("<username>", quote_plus(username)).replace("<password>", quote_plus(password))
    else:
        ...
        uri = f"mongodb://{quote_plus(username)}:{quote_plus(password)}@{host}:{port}/{target_db}?authSource={auth_db}"
...
def _connect_as_root(
    ...
    if not root_user or not root_pass:
        raise RuntimeError("MONGO_INITDB_ROOT_USERNAME and MONGO_INITDB_ROOT_PASSWORD must be set for root connections")

    host = host or os.getenv("DB_HOST") or "127.0.0.1"
    port = port or int(os.getenv("DB_PORT", "27017"))
    target_db = database or os.getenv("MONGO_INITDB_DATABASE") or os.getenv("DATABASE_NAME") or _DEFAULT_DB

    uri = f"mongodb://{quote_plus(root_user)}:{quote_plus(root_pass)}@{host}:{port}/{target_db}?authSource=admin"
    client: MongoClient = MongoClient(uri, serverSelectionTimeoutMS=server_selection_timeout_ms)
    client.admin.command("ping")
    return client, client[target_db]
...
class Connection:
        ...
        with zipfile.ZipFile(buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
            ...
            if mesh_path.suffix.lower() == ".obj":
                ...
                if mtl_path.is_file():
                    ...
                    logging.warning("No companion MTL file found for %s", mesh_path.name)
    ...
    def add_asset(self, asset: Asset) -> str:
        ...
        try:
            ...
            file_id = fs.put(zip_buf, filename=zip_filename, metadata={"mesh_id": asset_id})

            self.db["assets"].update_one({"_id": asset_id}, {"$set": {"mesh_file_id": file_id}})
    ...
    def extract_mesh_files(self, mesh_id: str, output_dir: str):
        if self.db is None:
            raise RuntimeError("Connection not open")
        try:
            asset_doc = self.db["assets"].find_one({"_id": ObjectId(mesh_id), {"mesh_file_id"}: 1, "name": 1})
            mesh_file_id = asset_doc.get("mesh_file_id")
            fs = gridfs.GridFS(self.db)
            grid_file = fs.get(mesh_file_id)
            output_path = Path(output_dir) / mesh_id
